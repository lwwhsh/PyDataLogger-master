...
class ScanMessenger(threading.Thread):
    ...
    def run(self):
        ...
        The thread will stop when .cpt == None or has not changed in
        a time  > .timeout
        """
        last_point = self.cpt
        t0 = time.time()
        while True:
            poll(MIN_POLL_TIME, 0.010)
            if self.cpt != last_point:
                last_point =  self.cpt
                t0 = time.time()
                # if self.cpt is not None and hasattr(self.func, '__call__'):
                if hasattr(self.func, '__call__'):
                    self.func(cpt=self.cpt, scan=self.scan, **self.func_kws)

            if self.cpt is None or time.time()-t0 > self.timeout:
                return

# change LWW for make thread. #class StepScan(object):
class StepScan(threading.Thread):
    """
    General Step Scanning for Epics
    """
    def __init__(self, filename=None, auto_increment=True,
                 comments=None, messenger=None):
        # add by LWW for make thread.
        threading.Thread.__init__(self)
        ...
        self.dwelltime = None
        # Add lww
        self.min_dwelltime=0.001 #second
    ...
    def run(self, filename=None, comments=None):
        ...
        # add by LWW for XAFS beamline, DCM need 1st move to start position - 200[ev]
        out = self.positioners[0].move_to(self.positioners[0].array[0] - 200,
                                          wait=True)
        #self.check_outputs(out, msg='move to start with -200')
        ...
        while not self.abort:
            ...
            try:
                ...
                # wait, then read counters and actual positions
                poll(self.det_settle_time, 0.010)

                [c.read() for c in self.counters]
                # print 'Read Counters done'
                #LWW disable, I think self.cdat is not use,
        ...
        for val, pos in zip(orig_positions, self.positioners):
            pos.move_to(val, wait=True) # LWW wait=False)

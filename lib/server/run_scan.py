...
def run_scanfile(scanfile='json.ini'):
...
def run_scan(conf):
    ...
    '''
    change by LWW, JSON parsing has error for tupel type
    so, changed 'tuple' type to list type.
    ...
    # LWW, disable this line and add next all lines # return scan
    scan.messenger  = report
    # scan.run(filename='out1.dat') #, comments=comments)
    scan.start(filename='out1.dat')
